...
class TicTacToeBot:
    ...
    def can_win(self, my_spots, open_spots):
        ...
        for pair in combinations(my_spots, 2):
            ...
            complement = (-(pair[0][0] + pair[1][0]) %
                          3, -(pair[0][1] + pair[1][1]) % 3)
    ...
    def get_curr_outcome(self, board, curr, level=0, verbose=False):
        ...
        for move in open_spots:
            ...
            if curr == "x":
                others_outcome = self.get_curr_outcome(
                    new_board, "o", level=level+1)[0]
            elif curr == "o":
                others_outcome = self.get_curr_outcome(
                    new_board, "x", level=level+1)[0]
            if others_outcome == 1:
        ...
        if level == 0 and verbose:
            print("win moves", currwin, "draw moves",
                  currdraw, "loose moves", currloose, sep='\n')
        if len(currwin) > 0:
            ...
            raise ValueError(
                f"current possible moves are \ncurrwin {currwin}\ncurrdraw {currdraw}\ncurrloose{currloose}")

    def move(self, board):
        outcome, move = self.get_curr_outcome(
            board, curr=self.player, verbose=True)
        if outcome == 1:
